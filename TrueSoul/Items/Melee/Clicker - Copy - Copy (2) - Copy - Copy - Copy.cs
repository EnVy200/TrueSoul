...
{
    public class ClickerUpgrade6 : ModItem
    {
        public override void SetStaticDefaults()
        {
            DisplayName.SetDefault("Titanium Clicker"); // By default, capitalization in classnames will add spaces to the display name. You can customize the display name here by uncommenting this line.
        ...
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            ...
            if (player.GetModPlayer<Playe>().Clicks >= 6)
            {
                Projectile.NewProjectileDirect(source, position, velocity, ModContent.ProjectileType<TitaniumAttack>(), damage * 6, knockback, player.whoAmI);
        ...
        public override void AddRecipes()
        {
            ...
            recipe.AddIngredient(ItemID.OrichalcumBar, 26);
            recipe.AddTile(TileID.Anvils);
            recipe.Register();

            Recipe recipe2 = CreateRecipe();
            recipe2.AddIngredient(ItemID.Wood, 66);
            recipe2.AddIngredient(ItemID.MythrilBar, 26);
